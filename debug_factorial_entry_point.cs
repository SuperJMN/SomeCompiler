...
// Si no se encontró en debug, buscar manualmente
if (entryPc == 0)
{
    var lines = asm.Split('\n');
    var mainLineIndex = -1;
    ...
    if (mainLineIndex >= 0)
    {
        // Buscar la primera instrucción RET para determinar donde termina la función anterior
        for (int pc = 0; pc < bin.Length - 1; pc++)
        {
            if (bin[pc] == 0xC9) // RET instruction
            {
                entryPc = (ushort)(pc + 1);
                Console.WriteLine($"Found RET at PC {pc:X4}, setting entry to {entryPc:X4}");
                break;
            }
        }
    }
